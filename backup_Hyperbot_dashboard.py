...
HYPERCOIN_CONTRACT = os.getenv("HYPERCOIN_CONTRACT")  # Set in .env
...
def login():
    ...
    user = data.get("user")
    if user:
        token = jwt.encode({"user": user, "exp": datetime.datetime.utcnow() + datetime.timedelta(hours=1)}, JWT_SECRET, algorithm="HS256")
